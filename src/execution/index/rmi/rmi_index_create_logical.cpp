...
PhysicalOperator &RMI::CreatePlan(PlanIndexInput &input) {
    auto &op = input.op;
	...
	auto &create_idx = planner.Make<PhysicalCreateRMIIndex>(op, op.table, op.info->column_ids, std::move(op.info),
